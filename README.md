...
跨境购物 Agent 工程原型，包含商品检索与重排、订单工具、偏好记忆、Redis 队列、FastAPI / WebSocket 事件流和 React 工作台。
...
基于 FDS 6.10.1 的隧道火灾研究代码，覆盖输入生成、结果审计、统计分析和测试，强调仿真证据的可追溯性。

### [Novel2Script](https://github.com/baoerger/novel-to-script)

中文小说转剧本系统，包含长文本分片、跨章角色去重、关系图谱、场景生成、结构化输出和质量报告。
...
`Python` · `Go` · `FastAPI` · `Redis` · `PostgreSQL` · `LangGraph` · `AgentScope` · `ReAct` · `Qdrant` · `OpenSearch` · `LLM-as-a-Judge` · `LangFuse` · `Codex` · `Claude Code`

早期项目历史整理在 [Legacy Labs](https://github.com/baoerger/legacy-labs)。
